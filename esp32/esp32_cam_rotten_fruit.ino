...
/* ThingsBoard Credentials ------------------------------------------------- */ // ← ADD THIS BLOCK
...
/* ThingsBoard MQTT Helper ------------------------------------------------- */ // ← ADD THIS FUNCTION
void sendToThingsBoard(float fresh_score, float rotten_score,
                       const char *status) {
...
void setup() {
  ...
  /* ThingsBoard MQTT setup */ // ← ADD THIS
...
void loop() {
  tbClient.loop(); // ← ADD THIS (keeps MQTT alive)
  ...
  for (uint16_t i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++) {
    if (strcmp(ei_classifier_inferencing_categories[i], "fresh") == 0)
      fresh_score = result.classification[i].value;
    if (strcmp(ei_classifier_inferencing_categories[i], "rotten") == 0)
      rotten_score = result.classification[i].value;
  }
  ...
  sendToThingsBoard(fresh_score, rotten_score, status); // ← ADD THIS
